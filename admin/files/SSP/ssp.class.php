...
$file = $_SERVER['DOCUMENT_ROOT'].'/smm/files/functions.php';
if ( is_file( $file ) ) {
	include( $file );
}
...
class SSP {
	...
	static function filter ( $request, $columns, &$bindings,$default = false )
	{
        ...
        $request['columns'] = isset($request['columns']) ? $request['columns'] : [];
        $keys = array_keys($request['columns']);
        // Individual column filtering
		for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
			$requestColumn = $request['columns'][$keys[$i]];
			$columnIdx = array_search( $requestColumn['data'], $dtColumns );
			$column = $columns[ $columnIdx ];

			$str = isset($requestColumn['search']) ? $requestColumn['search']['value'] : "";
	...
	static function simple ( $request, $sql_details, $table, $primaryKey, $columns,$default = false )
	{
        ...
        */
		 ...
		 * Output
		 */
        $request['draw'] = isset($request['draw']) ? $request['draw'] : 0;
		return array(
